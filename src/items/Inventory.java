...
public class Inventory implements Serializable {
	...
	public boolean add(Item item) {
		// first check if that item exists in the inventory
		for (int i = 0; i < items.length - DEFAULT_EQUIPMENT_LENGTH; i++) {
			if (items[i] != null && items[i].equals(item) && itemCounts[i] < items[i].maxCount) {
				itemCounts[i]++;
		...
		// inventory is full -- couldn't add it!
		return false;
	}
	...
	private BufferedImage getTileImage(int index) {
		...
		if (item != null) {
			...
			if (count > 1)
				g.drawImage(countImage, 10 - countImage.getWidth(), 9 - countImage.getHeight(), countImage.getWidth(),
						countImage.getHeight(), null);
		}
		return img;
	}
}
