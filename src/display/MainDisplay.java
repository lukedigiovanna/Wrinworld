...
public class MainDisplay extends Display {
	...
	public MainDisplay(Panel panel) {
		...
		this.add(new Button("update notes", Program.DISPLAY_WIDTH / 2, Program.DISPLAY_HEIGHT / 2 - 40 + 50, 40,
				new Runnable() {
					public void run() {
						DisplayController.setDisplay(DisplayController.UPDATE_NOTES_DISPLAY);
					}
				}));
		this.add(new Button("how to play", Program.DISPLAY_WIDTH / 2, Program.DISPLAY_HEIGHT / 2 - 40 + 100, 40,
				new Runnable() {
					public void run() {
						DisplayController.setDisplay(DisplayController.HOW_TO_PLAY_DISPLAY);
					}
				}));
		this.add(new Button("credits", Program.DISPLAY_WIDTH / 2, Program.DISPLAY_HEIGHT / 2 - 40 + 150, 40,
				new Runnable() {
					public void run() {
						DisplayController.setDisplay(DisplayController.CREDITS_DISPLAY);
					}
				}));
		this.add(new Button("quit", Program.DISPLAY_WIDTH / 2, Program.DISPLAY_HEIGHT / 2 - 40 + 200, 40,
				new Runnable() {
					public void run() {
						System.exit(0);
					}
	...
	public void repaint() {
		...
		g.setFont(new Font("Vivaldi", Font.BOLD | Font.ITALIC, 110));
