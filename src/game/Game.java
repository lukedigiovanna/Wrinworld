...
public class Game implements Serializable {
	...
	// an instance of a certain game.. contains things like the player data, world
	// data, progress etc.
	...
	// basically the constructor.. isn't called, though, until the game actually
	// starts and can be called again to reset.
	public void initiate() {
		...
		Spawner[] spawners = { new Spawner(0, 0, world.width() / 2, 0.005, new Pig(0, 0)),
		...
		Entity[] entityOrder = { new Slime(0, 0), new FireDevil(0, 0), new Minion(0, 0), new Zombie(0, 0),
				new Skeleton(0, 0), new SwordZombie(0, 0), new WildBoar(0, 0), new Wizard(0, 0), new Mage(0, 0),
				new Giant(0, 0) };
		...
		for (int i = 0; i < lootCount; i++) {
			entities.add(new LootBox(MathUtils.randomInRange(-this.world.width() / 2, this.world.width() / 2),
					MathUtils.randomInRange(-this.world.height() / 2, this.world.height() / 2)));
		}
		...
		for (int i = 0; i < 12; i++) {
			Trader t = new Trader(Trader.Preset.values()[(int) (Math.random() * Trader.Preset.values().length)],
					MathUtils.randomInRange(-25, 25.0), MathUtils.randomInRange(-25.0, 25.0));
			entities.add(t);
		}
		...
		date = new Date(60 * 60 * 7 + 60 * 60 * 24 * 150 + 60 * 12 + 10 * 60 * 60);
		...
		listeners.add(new MouseWheelListener() {
			...
			public void mouseWheelMoved(MouseWheelEvent e) {
				player().getInventory()
						.setSelectedSlot(player().getInventory().getSelectedSlot() - e.getWheelRotation());
				if (showQBGUI) {
	...
	public void update() {
		...
		entities.update();
		date.addTime(60 * 10 / Program.TICKS_PER_SECOND);
		questBook.update();
		console.update();
	...
	private BufferedImage img = new BufferedImage(Program.DISPLAY_WIDTH, Program.DISPLAY_HEIGHT,
			BufferedImage.TYPE_INT_ARGB);
	private static final BufferedImage GENERIC_HELMET = ImageProcessor
			.transparent(Program.getImage("items/armor/generichelmet.png"), 0.35),
			GENERIC_CHESTPLATE = ImageProcessor.transparent(Program.getImage("items/armor/genericchestplate.png"),
					0.35),
	...
	public Image getView() {
		...
		String[] list = { date.toString(),
				// "X: "+MathUtils.round(player.getX(),0.1)+" Y:
				// "+MathUtils.round(player.getY(),0.1),
				player.getHealth().toString(), player.getExperience().toString(), "$" + player.getMoney(),
		...
		g.drawImage(
				(player.getInventory().get(36) == null) ? GENERIC_HELMET : player.getInventory().get(36).getIconImage(),
				x, y, size, size, null);
		g.drawImage((player.getInventory().get(37) == null) ? GENERIC_CHESTPLATE
				: player.getInventory().get(37).getIconImage(), x, y += (size + padding), size, size, null);
		g.drawImage((player.getInventory().get(38) == null) ? GENERIC_LEGGINGS
				: player.getInventory().get(38).getIconImage(), x, y += (size + padding), size, size, null);
		g.drawImage(
				(player.getInventory().get(39) == null) ? GENERIC_BOOTS : player.getInventory().get(39).getIconImage(),
				x, y += (size + padding), size, size, null);
		y += (size + padding);
		...
		if (!guiManager.getCurrentID().equals("player_inventory")) {
			...
			g.drawImage(hotbarImage, img.getWidth() / 2 - hotbarDrawnWidth / 2, img.getHeight() - hotbarDrawnHeight * 2,
					hotbarDrawnWidth, hotbarDrawnHeight, null);
		}
		...
		if (PopupBar.queue.size() > 0 && PopupBar.queue.get(0) != null) {
			...
			g.drawImage(image, (int) (img.getWidth() * sideMars),
					img.getHeight() - image.getHeight() - (int) (img.getHeight() * sideMars), image.getWidth(),
					image.getHeight(), null);
		}

		if (showQBGUI) {
			BufferedImage qbIMG = qbGUI.getImage();
			x = img.getWidth() / 2 - qbIMG.getWidth() / 2;
			y = img.getHeight() / 2 - qbIMG.getHeight() / 2;
